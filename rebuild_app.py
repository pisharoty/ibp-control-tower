...
# 2. Pick the initial commit where gui_app.py had all 5 modules intact
base_commit = commits[-1]
print(f"Restoring base gui_app.py from initial commit: {base_commit}...")
subprocess.run(["git", "checkout", base_commit, "--", "gui_app.py"], check=True)

with open("gui_app.py", "r") as f:
    pristine_code = f.read()

# 3. CTRM Extension Payload (uses unique divider)
ctrm_extension = """
...
with open("gui_app.py", "w") as f:
    f.write(pristine_code.strip() + "\n" + ctrm_extension)

print("✅ Base app restored and CTRM engine cleanly attached!")
