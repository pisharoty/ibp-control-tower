...
if st.button("Run Black-76 Option Valuation & Arbitrage Solver", key="ctrm_btn"):
    ...
    try:
        res = requests.post(f"{BACKEND_URL}/api/v1/ibp/trading/black-scholes", json=payload, timeout=10)
...
if 'last_ctrm_res' in st.session_state and st.session_state['last_ctrm_res']:
    ...
    if st.button("💰 Commit Trade Yield to Corporate P&L Ledger", key="commit_pnl_btn"):
        ...
        try:
            commit_res = requests.post(f"{BACKEND_URL}/api/v1/ibp/trading/commit", json=commit_payload, timeout=10)
            if commit_res.status_code == 200:
                st.balloons()
                st.success("✅ Trade committed to persistent backend ledger!")
                st.rerun()
            else:
                st.error(f"Commit Failed ({commit_res.status_code}): {commit_res.text}")
        except Exception as e:
            st.error(f"Connection Error: {e}")

# Defensive check around audit table
if 'ledger_data' in locals() or 'ledger_data' in globals():
    if isinstance(ledger_data, dict) and len(ledger_data.get('trades', [])) > 0:
        st.markdown("---")
        st.subheader("📋 Active Corporate Trade & Derivatives Book")
        st.dataframe(ledger_data['trades'], use_container_width=True)


    # --- CTRM Commodity & Derivatives Trading Engine (Black-76) ---
    import math
    ...
    if st.button("Run Black-76 Option Valuation & Arbitrage Solver", key="ctrm_btn"):
        ...
        calculated = False
        try:
            res = requests.post(f"{BACKEND_URL}/api/v1/ibp/trading/black-scholes", json=payload, timeout=3)
            if res.status_code == 200:
                st.session_state['last_ctrm_res'] = res.json()
                calculated = True
        except Exception:
    ...
    if 'last_ctrm_res' in st.session_state and st.session_state['last_ctrm_res']:
        ...
        if st.button("💰 Commit Trade Yield to Corporate P&L Ledger", key="commit_pnl_btn"):
            ...
            try:
                commit_res = requests.post(f"{BACKEND_URL}/api/v1/ibp/trading/commit", json=commit_payload, timeout=3)
                if commit_res.status_code == 200:
                    st.balloons()
                    st.success("✅ Trade committed to persistent backend ledger!")
                    st.rerun()
