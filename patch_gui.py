...
# Cut off all stacked/appended CTRM blocks at the first append marker
markers = [
    "# =====================================================================",
    "# >>> CTRM_RISK_DESK_BLOCK <<<",
    "# UNIFIED RISK INJECTOR",
    "# INTERACTIVE RISK DISASTER SELECTOR",
    "from ctrm_engine import"
]

earliest_idx = len(code)
for m in markers:
    idx = code.find(m)
    if idx != -1 and idx < earliest_idx:
        earliest_idx = idx

base_code = code[:earliest_idx].strip()
