...
# 1. Search git log for the last commit where gui_app.py had full module logic
log_bytes = subprocess.check_output(["git", "log", "--oneline", "gui_app.py"])
log_lines = log_bytes.decode("utf-8").strip().split("\n")
commits = [line.split()[0] for line in log_lines if line.strip()]

target_commit = None
for c in commits:
    try:
        content = subprocess.check_output(["git", "show", f"{c}:gui_app.py"]).decode("utf-8")
        # Ensure commit contains actual module rendering logic
        if "Executive S&OP Dashboard" in content and "selected_module" in content and len(content) > 3000:
            target_commit = c
            print(f"Found intact 5-module commit: {c}")
            break
    except Exception:
        continue

if not target_commit:
    print("Fallback: Using git log commit index...")
    target_commit = commits[0]

# 2. Checkout intact base code
subprocess.run(["git", "checkout", target_commit, "--", "gui_app.py"], check=True)

with open("gui_app.py", "r") as f:
    clean_base = f.read()

# Strip any residual appended CTRM markers
for marker in ["# >>> UNIQUE_CTRM", "# ====================================================================="]:
    if marker in clean_base:
        # Only split if marker is past line 50 to avoid trimming header imports
        pos = clean_base.find(marker)
        if pos > 1000:
            clean_base = clean_base[:pos]

# 3. Append CTRM Risk Engine safely at the very bottom with guaranteed unique keys
ctrm_extension = """
...
print("✅ gui_app.py successfully restored and extended!")
