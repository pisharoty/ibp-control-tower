...
# 1. Fetch git commit log for gui_app.py
...
# Search recent commits for the modern 5-module Control Tower version
for c in commits:
    try:
        content = subprocess.check_output(["git", "show", f"{c}:gui_app.py"]).decode("utf-8")
        if "Executive S&OP Dashboard" in content and "Global Network & Logistics Map" in content:
            target_commit = c
            print(f"Found modern base commit: {c}")
            break
    except Exception:
        continue

if not target_commit:
    print("⚠️ Could not locate commit with modern modules via git show, restoring HEAD~1...")
    target_commit = "HEAD~1"

# 2. Checkout the true modern gui_app.py
subprocess.run(["git", "checkout", target_commit, "--", "gui_app.py"], check=True)

with open("gui_app.py", "r") as f:
    clean_code = f.read()

# Strip any previous CTRM appends if present in that commit
marker = "# >>> UNIQUE_CTRM_RISK_ENGINE_DESK"
if marker in clean_code:
    clean_code = clean_code.split(marker)[0]

# 3. Append CTRM Risk Engine safely to the bottom
ctrm_extension = """

# >>> UNIQUE_CTRM_RISK_ENGINE_DESK_v3 <<<
...
print("🚀 Successfully restored modern 5-module IBP app and attached CTRM engine!")
